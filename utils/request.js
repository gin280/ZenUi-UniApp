...
const zen = {
	...
	request: function(url, method, postData, isDelay, isForm, hideLoading) {
		//接口请求
		let loadding = false;
		zen.delayed && uni.hideLoading();
		clearTimeout(zen.delayed);
		zen.delayed = null;
		if (!hideLoading) {
			zen.delayed = setTimeout(() => {
				uni.showLoading({
					mask: true,
					title: '请稍候...',
					success(res) {
		...
		return new Promise((resolve, reject) => {
			uni.request({
				url: zen.interfaceUrl() + url,
				data: postData,
				header: {
					'content-type': isForm ? 'application/x-www-form-urlencoded' : 'application/json',
					'Authorization': zen.getToken()
				},
	...
	zenJsonp: function(url, callback, callbackname) {
		// #ifdef H5
		window[callbackname] = callback;
		let zenScript = document.createElement("script");
		zenScript.src = url;
		zenScript.type = "text/javascript";
		document.head.appendChild(zenScript);
		document.head.removeChild(zenScript);
		// #endif
	},
	//设置用户信息
