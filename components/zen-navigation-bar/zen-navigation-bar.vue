<template>
	<view
		class="zen-navigation-bar"
		:class="{ 'zen-bar-line': opcity > 0.85 && splitLine, 'zen-navbar-fixed': isFixed }"
		:style="{ height: height + 'px', backgroundColor: `rgba(${backgroundColor},${opcity})` }"
	>
		<view class="zen-status-bar" :style="{ height: statusBarHeight + 'px' }" v-if="isImmersive"></view>
		<view class="zen-navigation_bar-title" :style="{ opacity: opcity, color: color, paddingTop: top - statusBarHeight + 'px' }" v-if="title && !isCustom">{{ title }}</view>
		<slot />
	</view>
</template>
