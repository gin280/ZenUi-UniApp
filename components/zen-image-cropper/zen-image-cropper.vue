<template>
	<view class="zen-container" @touchmove.stop.prevent="stop">
		<view class="zen-image-cropper" @touchend="cutTouchEnd" @touchstart="cutTouchStart" @touchmove="cutTouchMove">
			<view class="zen-content">
				<view class="zen-content-top zen-bg-transparent" :style="{ height: cutY + 'px', transitionProperty: cutAnimation ? '' : 'background' }"></view>
				<view class="zen-content-middle" :style="{ height: canvasHeight + 'px' }">
					<view class="zen-bg-transparent" :style="{ width: cutX + 'px', transitionProperty: cutAnimation ? '' : 'background' }"></view>
					<view
						class="zen-cropper-box"
					...
					>
						<view
							...
							:key="index"
							class="zen-edge"
							:class="[`zen-${index < 2 ? 'top' : 'bottom'}-${index === 0 || index === 2 ? 'left' : 'right'}`]"
							:style="{
								width: edgeWidth,
								height: edgeWidth,
					...
					<view class="zen-flex-auto zen-bg-transparent" :style="{ transitionProperty: cutAnimation ? '' : 'background' }"></view>
				</view>
				<view class="zen-flex-auto zen-bg-transparent" :style="{ transitionProperty: cutAnimation ? '' : 'background' }"></view>
			</view>
			<image
		...
		<canvas
			canvas-id="zen-image-cropper"
			id="zen-image-cropper"
			:disable-scroll="true"
			:style="{ width: CROPPER_WIDTH * scaleRatio + 'px', height: CROPPER_HEIGHT * scaleRatio + 'px' }"
			class="zen-cropper-canvas"
		></canvas>
		<view class="zen-cropper-tabbar" v-if="!custom">
			<view class="zen-op-btn" @tap.stop="back">取消</view>
			<image :src="rotateImg" class="zen-rotate-img" @tap="setAngle"></image>
			<view class="zen-op-btn" @tap.stop="getImage">完成</view>
		</view>
	</view>
</template>
...
export default {
	...
	mounted() {
		...
		this.ctx = uni.createCanvasContext('zen-image-cropper', this);
		this.setCutCenter();
