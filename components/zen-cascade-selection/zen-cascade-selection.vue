<template>
	<view class="zen-cascade-selection">
		<scroll-view
			scroll-x
			scroll-with-animation
			:scroll-into-view="scrollViewId"
			:style="{ backgroundColor: headerBgColor }"
			class="zen-bottom-line"
			:class="{ 'zen-btm-none': !headerLine }"
		>
			<view class="zen-selection-header" :style="{ height: tabsHeight, backgroundColor: backgroundColor }">
				<view
					class="zen-header-item"
					:class="{ 'zen-font-bold': index === currentTab && bold }"
				...
				>
					{{ item.text }}
					<view class="zen-active-line" :style="{ backgroundColor: lineColor }" v-if="index === currentTab && showLine"></view>
				</view>
			</view>
		</scroll-view>
		<swiper class="zen-selection-list" :current="currentTab" duration="300" @change="switchTab" :style="{ height: height, backgroundColor: backgroundColor }">
			<swiper-item v-for="(item, index) in selectedArr" :key="index">
				<scroll-view scroll-y :scroll-into-view="item.scrollViewId" class="zen-selection-item" :style="{ height: height }">
					<view class="zen-first-item" :style="{ height: firstItemTop }"></view>
					<view
						class="zen-selection-cell"
					...
					>
						<icon type="success_no_circle" v-if="item.index === subIndex" :color="checkMarkColor" :size="checkMarkSize" class="zen-icon-success"></icon>
						<image :src="subItem.src" v-if="subItem.src" class="zen-cell-img" :style="{ width: imgWidth, height: imgHeight, borderRadius: radius }"></image>
						<view
							class="zen-cell-title"
							:class="{ 'zen-font-bold': item.index === subIndex && textBold, 'zen-flex-shrink': nowrap }"
							:style="{ color: item.index === subIndex ? textActiveColor : textColor, fontSize: textSize + 'rpx' }"
						>
							{{ subItem.text }}
						</view>
						<view class="zen-cell-sub_title" :style="{ color: subTextColor, fontSize: subTextSize + 'rpx' }" v-if="subItem.subText">{{ subItem.subText }}</view>
					</view>
				</scroll-view>
			</swiper-item>
		</swiper>
...
<style scoped>
.zen-cascade-selection {
	width: 100%;
	box-sizing: border-box;
}

.zen-selection-header {
	width: 100%;
	display: flex;
	align-items: center;
	position: relative;
