<template>
	<view class="zen-skeleton-cmomon zen-skeleton-box" :style="{width: winWidth+'px', height:winHeight+'px', backgroundColor:backgroundColor}">
		<view class="zen-skeleton-cmomon" v-for="(item,index) in skeletonElements" :key="index" :style="{width: item.width+'px', height:item.height+'px', left: item.left+'px', top: item.top+'px',backgroundColor: skeletonBgColor,borderRadius:getRadius(item.skeletonType,borderRadius)}"></view>
		<view class="zen-loading" :class="[getLoadingType(loadingType)]" v-if="isLoading"></view>
	</view>
</template>
...
<script>
	export default {
		...
		props: {
			...
			//默认所有，根据页面情况进行传值
			//页面对应元素class为：zen-skeleton-rect，zen-skeleton-circular，zen-skeleton-fillet
