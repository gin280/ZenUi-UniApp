<template>
	<view class="zen-rate-class zen-rate-box" @touchmove="touchMove">
		<block v-for="(item,index) in quantity" :key="index">
			<view class="zen-icon zen-relative" :class="['zen-icon-collection'+(hollow && (current<=index || (disabled && current<=index+1))?'':'-fill')]"
			 :data-index="index" @tap="handleTap" :style="{fontSize:size+'px',color:(current>index+1 || (!disabled && current>index))?active:normal}">
				<view class="zen-icon zen-icon-main zen-icon-collection-fill" v-if="disabled && current==index+1" :style="{fontSize:size+'px',color:active,width:percent+'%'}"></view>
			</view>
		</block>
	</view>
</template>
...
<style scoped>
	...
	.zen-icon-collection-fill:before {
		content: "\e6ea";
	}

	.zen-icon-collection:before {
		content: "\e6eb";
	}
