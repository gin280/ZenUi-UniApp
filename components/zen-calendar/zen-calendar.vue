<template>
	<view>
		<view :class="{ 'zen-bottom-popup': isFixed, 'zen-popup-show': isShow && isFixed }">
			<view class="zen-calendar-header" :class="{'zen-calendar-radius':radius}" @touchmove.stop.prevent="stop" v-if="isFixed">
				<view>日期选择</view>
				<view class="zen-iconfont zen-font-close" hover-class="zen-opacity" :hover-stay-time="150" @tap="hide"></view>
			</view>

			<view class="zen-date-box">
				<view class="zen-iconfont zen-font-arrowleft" :style="{color:yearArrowColor}" hover-class="zen-opacity"
				 :hover-stay-time="150" v-if="arrowType == 1" @tap="changeYear(0)"></view>
				<view class="zen-iconfont zen-font-arrowleft" :style="{color:monthArrowColor}" hover-class="zen-opacity"
				 :hover-stay-time="150" @tap="changeMonth(0)"></view>
				<view class="zen-date_time">{{ showTitle }}</view>
				<view class="zen-iconfont zen-font-arrowright" :style="{color:monthArrowColor}" hover-class="zen-opacity"
				 :hover-stay-time="150" @tap="changeMonth(1)"></view>
				<view class="zen-iconfont zen-font-arrowright" :style="{color:yearArrowColor}" hover-class="zen-opacity"
				 :hover-stay-time="150" v-if="arrowType == 1" @tap="changeYear(1)"></view>
			</view>
			<view class="zen-date-header">
				<view class="zen-date">日</view>
				<view class="zen-date">一</view>
				<view class="zen-date">二</view>
				<view class="zen-date">三</view>
				<view class="zen-date">四</view>
				<view class="zen-date">五</view>
				<view class="zen-date">六</view>
			</view>
			<view class="zen-date-content">
				<block v-for="(item, index) in weekdayArr" :key="index">
					<view class="zen-date"></view>
				</block>
				<view class="zen-date" :class="{'zen-opacity':openDisAbled(year,month,index+1),'zen-start-date':(type==2 && startDate==`${year}-${month}-${index+1}`) ||type==1,'zen-end-date':(type==2 && endDate==`${year}-${month}-${index+1}`) || type==1}"
				 :style="{backgroundColor:isFixed? getColor(index,1):'transparent'}" v-for="(item, index) in daysArr" :key="index"
				 @tap="dateClick(index)">
					<view class="zen-date-text" :style="{color:isFixed? getColor(index,2):getStatusData(3,index),backgroundColor: getStatusData(2,index)}">
						<view v-if="isFixed || !getStatusData(4,index)">{{ index + 1 }}</view>
						<view class="zen-custom-desc">{{getStatusData(1,index)}}</view>
						<text class="zen-iconfont zen-font-check" v-if="getStatusData(4,index)"></text>
					</view>
					<view class="zen-date-desc" :style="{color:activeColor}" v-if="type==2 && startDate==`${year}-${month}-${index+1}` && startDate!=endDate">{{startText}}</view>
					<view class="zen-date-desc" :style="{color:activeColor}" v-if="type==2 && endDate==`${year}-${month}-${index+1}`">{{endText}}</view>
				</view>
				<view class="zen-bg-month">{{month}}</view>
			</view>

			<view class="zen-calendar-op" v-if="isFixed" @touchmove.stop.prevent="stop">
				<view class="zen-calendar-result">
					<text>{{type==1?activeDate:startDate}}</text>
					<text v-if="endDate">至{{endDate}}</text>
				</view>
				<view class="zen-calendar-btn_box">
					<zen-button :type="btnType" height="72rpx" shape="circle" :size="28" @click="btnFix(false)">确定</zen-button>
				</view>
			</view>
		</view>

		<view class="zen-popup-mask" :class="[isShow ? 'zen-mask-show' : '']" @touchmove.stop.prevent="stop" v-if="isFixed"
		 @tap="hide"></view>
	</view>
</template>
<script>
...
<style scoped>
	...
	.zen-iconfont {
		font-family: "zenDateFont" !important;
		font-size: 36rpx;
		font-style: normal;
	...
	.zen-font-close:before {
		content: "\e608";
	}

	.zen-font-check:before {
		content: "\e6e1";
	}

	.zen-font-arrowright:before {
		content: "\e600";
	}

	.zen-font-arrowleft:before {
		content: "\e601";
	}
