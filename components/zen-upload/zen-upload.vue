<template>
	<view class="zen-container">
		<view class="zen-upload-box">
			<view class="zen-image-item" v-for="(item,index) in imageList" :key="index">
				<image :src="item" class="zen-item-img" @tap.stop="previewImage(index)" mode="aspectFill"></image>
				<view v-if="!forbidDel" class="zen-img-del" @tap.stop="delImage(index)"></view>
				<view v-if="statusArr[index]!=1" class="zen-upload-mask">
					<view class="zen-upload-loading" v-if="statusArr[index]==2"></view>
					<text class="zen-tips">{{statusArr[index]==2?'上传中...':'上传失败'}}</text>
					<view class="zen-mask-btn" v-if="statusArr[index]==3" @tap.stop="reUpLoad(index)" hover-class="zen-btn-hover"
					 :hover-stay-time="150">重新上传</view>
				</view>
			</view>
			<view v-if="isShowAdd" class="zen-upload-add" @tap="chooseImage">
				<view class="zen-upload-icon zen-icon-plus"></view>
			</view>
		</view>
	</view>
</template>
...
<style scoped>
	...
	.zen-upload-icon {
		font-family: "zenUpload" !important;
		font-style: normal;
	...
	.zen-upload-loading {
		...
		animation: zen-rotate 0.7s linear infinite;
	}
