<template>
	<view @touchmove.stop.prevent="stop">
		<view class="zen-bottom-navigation" :class="{ 'zen-navigation-fixed': isFixed, 'zen-remove-splitLine': unlined }">
			<view
				class="zen-navigation-item"
				:class="{ 'zen-item-after_height': splitLineScale, 'zen-last-item': index == itemList.length - 1 }"
				:style="{ backgroundColor: isDarkMode ? '#202020' : backgroundColor }"
				v-for="(item, index) in itemList"
				:key="index"
			>
				<view class="zen-item-inner" @tap="menuClick(index, item.parameter, item.type)">
					<image
						:src="current | getIcon(index, item)"
						class="zen-navigation-img"
						v-if="item.iconPath || (current == index && item.selectedIconPath && item.type == 1)"
					></image>
					<text
						class="zen-navigation-text"
						:style="{
				...
				<view
					class="zen-navigation-popup"
					:class="{ 'zen-navigation-popup_show': showMenuIndex == index }"
					:style="{ backgroundColor: isDarkMode ? '#4c4c4c' : subMenuBgColor, left: item.popupLeft || '50%' }"
					v-if="item.itemList"
				>
					<view
						class="zen-popup-cell"
						:class="{ 'zen-first-cell': subIndex === 0, 'zen-last-cell': subIndex === item.itemList.length - 1 }"
						:hover-class="subMenuHover ? (isDarkMode ? 'zen-item-dark_hover' : 'zen-item-hover') : ''"
						:hover-stay-time="150"
						v-for="(subItem, subIndex) in item.itemList || []"
						:key="subIndex"
						@tap="subMenuClick(index, item.type, subIndex, subItem.parameter)"
					>
						<text class="zen-ellipsis" :style="{ color: isDarkMode ? '#fff' : subMenuColor, fontSize: subMenufontSize, lineHeight: subMenufontSize }">
							{{ subItem.text }}
						</text>
					</view>
					<view class="zen-popup-triangle" :style="{ borderTopColor: isDarkMode ? '#4c4c4c' : subMenuBgColor }"></view>
				</view>
			</view>
		</view>
		<view class="zen-navigation-mask" :class="{ 'zen-navigation-mask_show': showMenuIndex != -1 }" @tap="handleClose"></view>
	</view>
</template>

<script>
export default {
	name: 'zenBottomNavigation',
	props: {
		//当前索引
		current: {
			type: Number,
