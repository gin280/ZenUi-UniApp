<template>
	<view @touchmove.stop.prevent>
		<view class="zen-fab-box" :class="{'zen-fab-right':!left || (left && right)}" :style="{left:getLeft(),right:getRight(),bottom:bottom+'rpx'}">
			<view class="zen-fab-btn" :class="{'zen-visible':isOpen,'zen-fab-hidden':hidden}">
				<view class="zen-fab-item-box" :class="{'zen-fab-item-left':left && !right && item.imgUrl}" v-for="(item,index) in btnList"
				 :key="index" @tap.stop="handleClick(index)">
					<view :class="[left && !right?'zen-text-left':'zen-text-right']" v-if="item.imgUrl" :style="{fontSize:item.fontSize+'rpx',color:item.color}">{{item.text || ""}}</view>
					<view class="zen-fab-item" :style="{width:width+'rpx',height:height+'rpx',background:item.bgColor || bgColor,borderRadius:radius}">
						<view class="zen-fab-title" v-if="!item.imgUrl" :style="{fontSize:item.fontSize+'rpx',color:item.color}">{{item.text || ""}}</view>
						<image :src="item.imgUrl" class="zen-fab-img" v-else :style="{width:item.imgWidth+'rpx',height:item.imgHeight+'rpx'}"></image>
					</view>
				</view>
			</view>
			<view class="zen-fab-item" :class="{'zen-active':isOpen}" :style="{width:width+'rpx',height:height+'rpx',borderRadius:radius,background:bgColor,color:color}"
			 @tap.stop="handleClick(-1)">
				<view class="zen-fab-icon zen-icon-plus"></view>
			</view>
		</view>
		<view class="zen-fab-mask" :class="{'zen-visible':isOpen}" @tap="handleClickCancel"></view>
	</view>
</template>
