    ...
    <zen-custom bgColor='bg-blue-200' :isBack='true'>
      ...
      <block slot="content">城市选择</block>
		...
		<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight+'px'}">
				...
				<view class="search-bar-form">
					<view class="search-bar-box">
						...
						<view class="icon-clear" v-if="inputVal" @tap="clearInput">
							...
							<!-- #endif -->
							<!-- #ifdef H5 -->
							<zen-icon name="close-fill" :size="16" color="#bfbfbf"></zen-icon>
							<!-- #endif -->
						</view>
					</view>
			...
			<view class="zen-list search-result" v-if="inputShowed">
				<view class="zen-list-cell" hover-class="zen-list-cell-hover" v-for="(item,index) in searchResult" :key="index"
				 @tap="selectCity" :data-name="item" :hover-stay-time='150'>
					<view class="zen-list-cell-navigate">
						{{item}}
					</view>
				</view>
			</view>
			<view v-if="!inputVal">
				<view class="current-city">
					...
					<view class="city-name">
						<zen-icon name="position-fill" color="#5677fc" :size="18"></zen-icon>
						{{localCity}}
					</view>
				</view>
				<view class="hot-city">
				...
				<view class="zen-list city-list">
					<block v-for="(list,index) in lists" :key="index" v-if="list.data[0]">
						<view class="zen-list-cell-divider" :id="index === 0 ? 'suoyin' : list.letter">
							{{list.letter}}
						</view>
						<view class="zen-list-cell" hover-class="zen-list-cell-hover" v-for="(item,index2) in list.data" :key="index2"
						 @tap="selectCity" :data-name="item.cityName" :hover-stay-time='150'>
							<view class="zen-list-cell-navigate" :class="[list.data.length-1==index?'last':'']">
								{{item.cityName}}
							</view>
						</view>
					</block>
		...
		<view class="zen-indexed-list-bar" :style="{height:indexBarHeight+'px'}" @touchstart="touchStart" @touchmove.stop="touchMove"
		 @touchend.stop="touchEnd" @touchcancel.stop="touchCancel" v-if="!inputVal">
			<text v-for="(items,index) in lists" :key="index" class="zen-indexed-list-text" :style="{height:indexBarItemHeight+'px'}">
				{{index==0?"索引":items.letter}}
			</text>
		</view>
		<view class="zen-indexed-list-alert" v-if="touchmove && lists[touchmoveIndex].letter">
...
<style>
	...
	.zen-indexed-list-text {
		font-size: 22rpx;
		white-space: nowrap;
	}

	.zen-indexed-list-bar.active {
		background-color: rgb(200, 200, 200);
	}

	.zen-indexed-list-alert {
		position: absolute;
		z-index: 20;
		width: 160rpx;
		height: 160rpx;
